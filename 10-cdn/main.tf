resource "aws_cloudfront_distribution" "devopstraining" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.devopstraining.domain_name
        zone_id = aws_cloudfront_distribution.devopstraining.hosted_zone_id
      }
    }
  ]
}